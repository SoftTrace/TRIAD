...
public class ExtractLibestBiterm {
    ...
    private static Map<String, Map<String, Integer>> level2CodeBitermsNumMap;
    private static Map<String, Map<String, Integer>> initLevel2CodeBitermsNumMap;
    ...
    private static Map<String, List<List<String>>> initLevel1BitermsMap;
    private static Map<String, List<List<String>>> initLevel2BitermsMap;
    private static Map<String, List<List<String>>> initLevel3BitermsMap;

    private static Set<String> initLevel3BitermsSet;
    private static Set<String> initCoreLevelBitermsSet;
    private static ArtifactLevelEnum coreLevel;
    ...
    public static void main(String[] args) {
        coreLevel = ArtifactLevelEnum.LEVEL2;
        ...
        extractLibestBiterm.outputBiterms(level1BitermsMap, libEST.getCoreLevelBitermPath(coreLevel, ArtifactLevelEnum.LEVEL1));
        extractLibestBiterm.outputBiterms(level2BitermsMap, libEST.getCoreLevelBitermPath(coreLevel, ArtifactLevelEnum.LEVEL2));
        extractLibestBiterm.outputBiterms(level3BitermsMap, libEST.getCoreLevelBitermPath(coreLevel, ArtifactLevelEnum.LEVEL3));
    }

    private void initBitermDirectory() {
        System.out.println("init");
        FileIOUtil.initDirectory(libEST.getCoreLevelBitermPath(coreLevel, ArtifactLevelEnum.LEVEL1));
        FileIOUtil.initDirectory(libEST.getCoreLevelBitermPath(coreLevel, ArtifactLevelEnum.LEVEL2));
        FileIOUtil.initDirectory(libEST.getCoreLevelBitermPath(coreLevel, ArtifactLevelEnum.LEVEL3));
    }

    public ExtractLibestBiterm() {
        this.libEST = new LibEST();
        this.level2CodeLayerBitermsNumMap = new HashMap<>();
        this.level2CodeBitermsNumMap = new HashMap<>();
        this.level2CodeLayerBiterms = new HashMap<>();
        this.initLevel2CodeBitermsNumMap = new HashMap<>();
        ...
        this.initLevel1BitermsMap = new HashMap<>();
        this.initLevel2BitermsMap = new HashMap<>();
        this.initLevel3BitermsMap = new HashMap<>();

        initLevel3BitermsSet = new HashSet<>();
    ...
    private void extractBiterms() {
        ...
        initCoreLevelBitermsSet = getInitCoreBitermSetFromsCode(initLevel2BitermsMap,initLevel2CodeBitermsNumMap);

        level1BitermsMap = getTextTypeBitermMap(libEST.getUnprocessedLevel1ArtifactPath(), false);

        initLevel3BitermsMap = getBitermFromClsMap(level3CodeLayerBitermsNumMap,level3CodeLayerBiterms);
        level3BitermsMap = getFinalClassBitermFromCoreMap(level3CodeLayerBitermsNumMap, initCoreLevelBitermsSet);

        level2BitermsMap = getFinalClassBitermMap(level2CodeLayerBitermsNumMap, level2CodeBitermsNumMap, level1BitermsMap, level3BitermsMap);
    }

    private static Set<String> getInitCoreBitermSetFromsCode(Map<String, List<List<String>>> initCoreBitermsListMap,Map<String, Map<String, Integer>> initCoreBitermsMap) {
        ...
        for (String cls : initCoreBitermsListMap.keySet()) {
            ...
            initCoreBitermsMap.put(cls,bitermNumMap);
            System.out.println(cls +" "+bitermNumMap.size());
    ...
    private static Map<String, Map<String, Integer>> getFinalClassBitermMap(
            ...
            Map<String, Map<String, Integer>> codeBitermsNumMap,
            Map<String, Map<String, Integer>> level1BitermNumMap,
            Map<String, Map<String, Integer>> level2BitermNumMap) {

        Set<String> level12BitermSet = new HashSet<>();
        for (String artifact : level1BitermNumMap.keySet()) {
            Map<String, Integer> map = level1BitermNumMap.get(artifact);
            level12BitermSet.addAll(map.keySet());
        }
        for (String artifact : level2BitermNumMap.keySet()) {
            Map<String, Integer> map = level2BitermNumMap.get(artifact);
            level12BitermSet.addAll(map.keySet());
        }
        for (String clazz : codeLayerBitermNumMap.keySet()) {
            ...
            for (String biterm : tmpMap.keySet()) {
                if (level12BitermSet.contains(biterm))
    ...
    private static Map<String, Map<String, Integer>> getTextTypeBitermMap(String reqDirPath, Boolean isCore) {
                            ...
                            for (String tp : tpList) {
                                if (!tp.equals("")) {
                                    ...
                                    if(isCore) {
                                        ...
                                        if (initCoreLevelBitermsSet.contains(biterm1)) {
                                            numMap.put(biterm1, numMap.getOrDefault(biterm1, 0) + 1);
                                        }
                                        ...
                                        if (initCoreLevelBitermsSet.contains(biterm2)) {
    ...
    private static Map<String, List<List<String>>> expandBiterm(Map<String, List<List<String>>> map,
        ...
        for (File file : dir.listFiles()) {
            ...
            for(String currentLine : list) {
                System.out.println(clazz+" "+currentLine);
